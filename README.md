...
3. Launch Raspberry Pi Imager
   i. Select operating system (e.g. Raspberry Pi OS (32-bit))
   i. Select Storage (your microSD card) e.g. "Generic SD/MMC Media"
   i. Click the gear icon to edit settings (click "No" if asked to use Keychain)
   i. Set hostname following our naming convention, e.g. "pi02a", "pi02b", "pi03a"...
   i. Check to "Enable SSH", and "Use password authentication"
   i. Set username and password to "sccn" and "marmoset"
   i. Check "Configure wireless LAN"
   i. Set SSID to "squirtle" and password to "marmomarm"
   i. Set "Wireless LAN country" to "US"
   i. Check "Set locale settings", set to "America/New York" and "US" keyboard layout
...
14. Log in to the Pi from the Mac via ssh: `ssh sccn@<newhostname>.local`
    i. Type "yes" to confirm
    i. Enter the password "marmoset" for the "sccn" user
    i. Verify that you're logged in to the new host, you should see the command prompt change to "sccn@<newhostname>"
...
16. Use command `ssh-copy-id -i ~/.ssh/id_rsa.pub sccn@<newhostname>.local` to copy the Mac's ssh public key to <newhostname>
    i. Enter the password for "sccn@<newhostname>.local"
    i. Confirm the command executed successfully
    i. Now log in again via ssh: `ssh sccn@<newhostname>.local`
    i. Confirm that no password was required (you logged in using public/private key pair authentication)
...
19. Enable the camera interface
    i. Select "3 Interface Options", press Return
    i. Select "I1 Legacy Camera", press Return
    i. Select "Yes" to enable legacy camera support
    i. Select "Ok"
    i. Select "Finish" and press Return
    i. Select "Yes" to reboot now (NOTE: this is necessary!)
