...
class TaskInterface(TaskStructure):
    ...
    def build_trial(self, trial_parameters={}):
        ...
        reward_stim = Stimulus(shape=StimulusShape.IMAGE, size=(250,250), image = 'tasks/images/composite6-1.jpg', color = (1,1,1), size_touch=(250,250)) #will need to change path
        penalty_stim = Stimulus(shape=StimulusShape.IMAGE, size=(250,250), image = 'tasks/images/composite6-2.jpg', color = (1,1,1), size_touch=(250,250)) #will need to change path
        reward_stim.position = trial_parameters[Parameter.POSITION]
        penalty_stim.position = self.randomize_from(self.pseudorandom_parameters[Parameter.POSITION]['values'], exclude=[trial_parameters[Parameter.POSITION]])
