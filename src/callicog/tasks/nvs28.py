	...
	def build_trial(self, trial_parameters={}):
		...
		if len(distractor_positions) == 11:
				for position in distractor_positions:
					...
					distractor_stim.outcome = Outcome.FAIL
					w3.add_stimulus(distractor_stim)
		elif len(distractor_positions) == 12:
				for position in distractor_positions:
					distractor2 = self.randomize_from(self.pseudorandom_parameters[Parameter.BLUESHAPES]['values'], size=12)
